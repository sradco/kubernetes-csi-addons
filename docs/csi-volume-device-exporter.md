...
| `--host-kubelet` | `/host/kubelet` | Path to host kubelet root mount inside container (requires `mountPropagation: HostToContainer`) |
...
1. **Kubelet Discoverer** — Parses `vol_data.json` from kubelet's CSI plugin
   directory and uses `stat()` to detect mount boundaries and resolve block
   devices via `/sys/dev/block/`. Requires `mountPropagation: HostToContainer`
   on the kubelet hostPath mount so CSI staging mounts are visible.
